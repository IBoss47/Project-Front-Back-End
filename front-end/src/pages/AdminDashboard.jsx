...
const AdminDashboard = () => {
      ...
      try {
        ...
        if (statsResponse.data.success) {
          ...
          setDashboardStats(prev => ({
            ...
            totalSalesAmount: stats.total_sales_amount || 0
          }));
        }
  ...
  const handleAddSellerRole = async (userId) => {
    try {
      ...
      if (response.data.success) {
        showSuccessMessage('เพิ่ม role seller สำเร็จ!');
  ...
  const handleApproveNote = async (noteId) => {
    try {
      ...
      if (response.data.success) {
        showSuccessMessage('อนุมัติสรุปสำเร็จ!');
  ...
  const handleRejectNote = async (noteId) => {
    ...
    try {
      ...
      if (response.data.success) {
        showSuccessMessage('ปฏิเสธสรุปสำเร็จ!');
        // บันทึก active tab ก่อน reload
        localStorage.setItem('adminActiveTab', activeTab);
        setTimeout(() => window.location.reload(), 1500);
      }
  ...
  const DraggableSliderImage = ({ image, index, moveImage, removeImage, updateLink }) => {
    ...
    const [localLinkValue, setLocalLinkValue] = useState('');

    // Initialize local state from image prop
    useEffect(() => {
      const currentLink = image.link_url?.String || image.link_url || '';
      setLocalLinkValue(currentLink);
    }, [image.id, showLinkInput]); // Only update when opening the input or image changes

    const handleLinkChange = (e) => {
      const newLink = e.target.value;
      setLocalLinkValue(newLink);
    };

    const handleSaveLink = () => {
      updateLink(image.id, localLinkValue);
      setShowLinkInput(false);
    };

    const displayLink = image.link_url?.String || image.link_url || '';

    return (
      <div
        ref={(node) => !showLinkInput && drag(drop(node))}
        className={`relative group ${isDragging ? 'opacity-50' : 'opacity-100'}`}
        style={{ cursor: showLinkInput ? 'default' : 'move' }}
      >
        <img
        ...
        <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            ...
            className="bg-blue-500 text-white rounded-full w-8 h-8 flex items-center justify-center hover:scale-110 transition-transform"
            title="ตั้งลิงก์"
          >
            🔗
          </button>
        ...
        {showLinkInput && (
          ...
          >
            <div className="w-full">
              <label className="text-white text-sm font-bold mb-2 block">ลิงก์ URL:</label>
              <input
                type="text"
                value={localLinkValue}
                onChange={handleLinkChange}
                placeholder="/SellListPage หรือ https://example.com"
              ...
              <div className="flex gap-2 mt-2">
                <button
                  ...
                  className="flex-1 px-3 py-2 bg-green-600 hover:bg-green-700 text-white rounded font-bold"
                >
                  ✓ บันทึก
                </button>
                <button
                  ...
                  className="px-3 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded font-bold"
                >
                  ✕
                </button>
              </div>
  ...
  const saveAllSliders = async () => {
    ...
    try {
      ...
      for (const newImage of newImages) {
        ...
        if (newImage.link_url) {
          formData.append('link_url', newImage.link_url);
        }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-800 to-gray-900 pt-24 pb-16">
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="mb-8">
          <button
            onClick={() => navigate(-1)}
            className="flex items-center gap-2 px-4 py-2 mb-4 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-colors"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6 mb-8">
          ...
          <div className="bg-gradient-to-br from-yellow-600 to-yellow-700 rounded-2xl shadow-lg p-6 text-white border border-yellow-500">
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-yellow-100 text-sm font-semibold mb-1">รายได้ทั้งหมด (2% จากยอดขาย)</p>
          ...
          <div className="bg-gradient-to-br from-emerald-600 to-emerald-700 rounded-2xl shadow-lg p-6 text-white border border-emerald-500">
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-emerald-100 text-sm font-semibold mb-1">ยอดขายทั้งหมด</p>
                <p className="text-3xl font-bold">฿{dashboardStats.totalSalesAmount.toFixed(2)}</p>
              </div>
              <ShoppingBagIcon className="w-14 h-14 opacity-30" />
            </div>
            <p className="text-emerald-100 text-sm">จากการซื้อขายในระบบ</p>
          </div>
          ...
          <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl shadow-lg p-6 text-white border border-orange-400">
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-orange-100 text-sm font-semibold mb-1">⏳ รอการอนุมัติ</p>
                <p className="text-4xl font-bold">{dashboardStats.pendingApprovals}</p>
              </div>
              <CheckCircleIcon className="w-14 h-14 opacity-30" />
            </div>
